...
export const validar_Sesion_Por_Id = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const validar_Sesion_Existente = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const validar_Sesion_body = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  await body("nombre_Sesion")
    .notEmpty()
    .withMessage("El nombre de la sesión es obligatorio")
    .isLength({ max: 255 })
    .withMessage("El nombre de la sesión no puede superar los 255 caracteres")
    .run(req);
