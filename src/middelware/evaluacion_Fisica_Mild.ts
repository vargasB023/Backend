...
export const validar_EvaluacionFisica_body = async ( req: Request, res: Response, next: NextFunction) => {
  ...
  // validar el ID_Evaluacion_De 
  await body('ID_Evaluacion_De')
    .notEmpty().withMessage('El ID de evaluación deportiva es obligatorio')
    .isInt({ min: 1 }).withMessage('El ID de evaluación deportiva debe ser un número válido')
    .run(req);
  ...
  // Hacer el IMC opcional ya que se calcula automáticamente
  await body('imc')
    .optional()
  ...
  await body('test_Course_Navette')
    .optional()
    .isFloat({ min: 0, max: 30 }).withMessage('El valor del test Navette debe estar entre 0.0 y 30.0')
    .run(req);
...
export const validar_Evaluacion_Deportiva_Referenciada = async ( req: Request, res: Response, next: NextFunction) => {
  ...
  // Solo validar si existe el ID_Evaluacion_De
  if (ID_Evaluacion_De) {
